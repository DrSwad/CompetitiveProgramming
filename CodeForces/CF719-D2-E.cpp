/*
	...
	Idea:
		- We'll use the matrix exponentiation technique used to get the n'th
		  Fibonacci number in O(logn):
		  	If A = (1 1) and b = (1)
		  	       (1 0)         (0)
		  	then A^(n - 1) * b = (F(n)    )
		  	                     (F(n - 1))
		- So, we'll process the queries using lazy segment tree. At first,
		  each leaf node of the tree for the  corresponding array index i
		  will contain A^(a_i - 1).

		- Then, to process queries of type 1, we'll multiply the segment
		  [l, r] by A^x. And to process queries of type 2, we'll get
		  the sum of the matrices in segment [l, r] and multiply it by
		  b to find a 2x1 matrix. Our answer lies in the first row of
		  this matrix.
*/
